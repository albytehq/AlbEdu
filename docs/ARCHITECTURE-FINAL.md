...
## v0.816.0 Hardening Summary

After the three-stage refactor captured above (Stage 1–3), the v0.816.0 cycle executed four parallel audits and applied a substantial set of stability and hardening fixes. This section summarizes what changed and why. For the human-style commenting rules that AI assistants MUST follow when editing this codebase, see [`docs/STRICT-COMMENTING-FOR-AI.md`](./STRICT-COMMENTING-FOR-AI.md).
...
- **image-cleanup timeouts** — `src/utils/image-cleanup.js` now wraps each cleanup call in a 10s `AbortController` timeout so a hung R2 response doesn't block the cleanup queue.
...
- **Worker soft-archive (was hard-delete)** — the Cloudflare Worker `/release` endpoint was hard-deleting R2 assets when an admin deleted an assessment. For an exam platform this is unacceptable — audit and forensic investigations often need to recover the original assessment images months later. The endpoint now sets `deleted_at` (soft-archive); permanent deletion is deferred to the 365-day retention pg_cron job.
...
- **Clarification (v0.816.0):** The original Stage 2 plan was to delete `public/QNotify/` and replace it with `src/shared/notify.js`. However, in practice, `public/QNotify/` is still present (20 files) and actively loaded via `src/shared/qnotify-loader.js` from multiple HTML pages (3+ admin pages + others). The planned `src/shared/notify.js` was never built.
...
- **Note (v0.816.0):** The planned `src/legacy/firebase-compat.js` bridge was never built — `src/legacy/` directory does not exist. All consumers were migrated directly to the native platform layer. A stale comment in `src/pages/results-analytics.js:11` references `firebase-compat.js` but the file does not exist (dead reference, harmless).
...
- ~~`src/pages/question-bank.js`~~ — **REMOVED in v0.816.0** (Bank Soal feature deleted, migration 019: `DROP TABLE question_bank CASCADE`).

### Still pending migration (tracked for future sprints)
**Note (v0.816.0):** The legacy bridge `src/legacy/firebase-compat.js` was never built, so these files were migrated directly to the native platform layer. The list below is preserved for historical reference; current status may differ — verify with `grep -rn "firebaseDb\|firebaseAuth" src/` before acting.
...
**Result: 15/15 checks passed, 0 errors, 0 warnings** (verified v0.816.0).
...
**Note (v0.816.0):** `src/legacy/` directory does NOT exist. The planned `firebase-compat.js` bridge was never built. All consumers were migrated directly to the native platform layer. QNotify vendor library is still present in `public/QNotify/` (20 files) and actively loaded via `src/shared/qnotify-loader.js`.
...
- `src/pages/{take-assessment,daftar-nama}.js` — migrated. ~~`question-bank.js`~~ — removed in v0.816.0 (Bank Soal feature deleted).
