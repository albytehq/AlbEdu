...
> AlbEdu v0.821.0 enterprise-grade anti-cheat: server-side scoring, heartbeat, DevTools detection, instant block.
...
| # | Attack | v0.2.0 Vulnerability | v0.821.0 Mitigation |
|---|---|---|---|
...
- `access-code-attempt` — token entry (added v0.821.0)
...
- [x] verify_jwt=true for all authenticated Edge Functions (v0.821.0)
- [x] RLS session-ownership check on rate_limit_heartbeats / rate_limit_submits / violation_events (v0.821.0)
- [x] peran_user() filters deleted_at (v0.821.0)
- [x] Cloudflare Worker /upload + /release locked to ALLOWED_ORIGINS + AUTH_TOKEN required (v0.821.0)
- [x] Worker soft-archive replaces hard-delete (v0.821.0)
- [x] Atomic submit_assessment() RPC (v0.821.0)
- [x] Consent previousVersion XSS escaped (v0.821.0)
- [x] PII leak fixed — auth/main.js no longer logs user.email (v0.821.0)
...
### Worker soft-archive replaces hard-delete — ⚠️ CORRECTION (v0.821.0)
...
## Security Roadmap & Incident Response

- **[Security Roadmap](./security/SECURITY-ROADMAP.md)** — Comprehensive remediation plan based on 4 parallel security audits (SEC-A through SEC-D). 35 vulnerabilities found (8 CRITICAL, 12 HIGH, 15 MEDIUM). Phased plan over 4 weeks.
- **[Incident Response Plan](./security/INCIDENT-RESPONSE.md)** — SEV-1 through SEV-4 response procedures, key rotation runbook, disaster recovery, communication templates.

**Critical finding (fix immediately):** A peserta can escalate to admin via `UPDATE users SET peran='admin'` — see [SECURITY-ROADMAP.md Phase S0 #1](./security/SECURITY-ROADMAP.md#phase-s0--emergency-fixes-48-hours--v08191--v08200).
