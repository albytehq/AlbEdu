...
## v0.819.0 Hardening Verified

The following edge cases were uncovered by the v0.819.0 stability audit and are now handled. Each one was a real bug that would have manifested in production under specific user conditions — they are NOT hypothetical. Re-verify each after any refactor that touches the listed file.
