...
>
> **Version:** v0.818.0  |  **Last updated:** 2026-07-08  |  **Owner:** Albi Fahriza (albytehq)
...
2. **Never use `href="/"`** anywhere. It jumps to `https://albytehq.github.io/` (the user's GitHub profile page), not the AlbEdu app. Use `href="./"` from root, `href="../"` from `pages/`, `href="../../"` from `pages/admin/` and `pages/assessment/` (flat structure).
...
7. **Asset paths in HTML are page-relative** (`../styles/x.css` from `pages/foo.html`, `../../styles/x.css` from `pages/admin/foo.html` or `pages/assessment/foo.html`). The root `index.html` and root `404.html` use bare paths (`styles/x.css`).
...
AUTH_CONFIG = {
  BASE_PATH: (function () {
    ...
    const APP_SUBFOLDERS = [
      ...
      // '/pages/' is required — without it, BASE_PATH returned
...
| Version | Date | Change |
|---|---|---|
| v0.818.0 | 2026-07-08 | **UI Hardening — AlbEdu Enterprise Component System**: Built `styles/albedu-components.css` — global UI component library (Spinner, Skeleton, Modal, Select-wrap, Checkbox, Radio, Alert, Badge, Empty-state, Tabs, Table, OTP, PG-option no-hover). Fixed tokens.css dark mode (was only flipping 5/19 AlbEdu tokens via [data-theme], now flips all via both [data-theme="dark"] AND @media prefers-color-scheme:dark). Standardized border-radius to 6/8/999/50% (was 14 distinct values 1-24px). Fixed broken `submit_assessment()` RPC in migration 021 (referenced non-existent columns + wrong type-check) → renamed to `submit_assessment_atomic()` with correct signature, scoring stays in Edge Function. Landing page: removed trust claims, migrated buttons to .albedu-btn*. Auth pages: removed "Terpercaya"/"Terjamin" claims, reordered login (Google-first), added 4 .albedu-alert banners (peserta=Google-only, registration=admin-only), login.css cleanup (2022→1109 lines, -913 dead duplicates, +dark mode). Admin panel: wrapped raw <select> in monitoring+results-analytics with .albedu-select-wrap, fixed Question Editor Modal (13 inline styles → canonical .albedu-modal*), migrated bespoke classes (.albedu-soal-textarea→.albedu-textarea, .albedu-field-input→.albedu-input, .albedu-section-type→.albedu-select), added dashboard skeleton+empty-state, wired panel.js. Exam runtime: removed PG option hover (clean look mobile+desktop per user request), always show section tabs (was hidden when 1 section), renamed esai-textarea→albedu-textarea, bumped .page-tab to 44px touch target. Forms: migrated 8 auth inputs .input→.albedu-input, fixed daftar-nama touch targets (32px→44px), fixed .field-password .input selector in 3 auth CSS. Dark mode: added @media prefers-color-scheme:dark blocks to 8 CSS files (admin-panel, navigasi, notification-panel, daftar-nama, active-assessments, monitoring, results-analytics, take-assessment). 4 parallel subagent audits (exam system deep study + landing/auth + admin/exam + forms/dark/empty). All JS pass node --check. verify-structure: 0 errors. |
| v0.818.0 | 2026-07-08 | **Stability hardening + AI-pattern cleanup + doc overhaul**: Bumped package.json from 0.746.0 to v0.818.0. Aligned manifest.json + service-worker CACHE_VERSION + 8 admin HTML footers + option-profile footer + README badge + docs (README, ARCHITECTURE-FINAL, AI-CONTEXT, USER-SIMULATION-CHECKLIST, SECURITY, SCALING, CONTRIBUTING, THEME-SYSTEM, COMPLIANCE) + supabase/README + rule-url-albedu to v0.818.0. Created new doc `docs/STRICT-COMMENTING-FOR-AI.md` codifying the 5 human-style commenting rules (kill ASCII art, version archaeology, marketing-speak, JSDoc on internals). Added v0.818.0 sections to ARCHITECTURE-FINAL (Hardening Summary — 4 parallel audits, ~1,400+ AI patterns killed, Free Plan capacity corrected to ~100 concurrent peserta), AI-CONTEXT (atomic submit_assessment RPC, heartbeatDbCache 60s, 4-arg realtime.subscribe), USER-SIMULATION-CHECKLIST (12 edge cases verified), SECURITY (RLS tightening on rate_limit_heartbeats/submits + violation_events session-ownership check, peran_user deleted_at filter, verify_jwt=true for 8 functions, Cloudflare Worker CORS lock + AUTH_TOKEN required, Worker soft-archive replaces hard-delete, PII leak fixed, consent XSS escaped), SCALING (corrected Free Plan ceilings + keep-warm strategy), CONTRIBUTING (new "Before You Edit" section pointing to STRICT-COMMENTING-FOR-AI.md). Stale v0.742.x archaeology comments in docs replaced with current references where they appeared as "since v0.742.0+" notes (changelog history entries preserved as historical records). |
