...
// benchmark_icons.mjs — performance benchmark for the icon system.
// Measures cold/warm render, critical vs secondary path, bulk bindIcons()
// throughput, cache effectiveness, and (if available) legacy comparison.
...
console.log('AlbEdu Icon System — Performance Benchmark Suite');
...
// BENCHMARK 7: Compare against legacy baseline if available

console.log('\nBenchmark 7: current vs legacy comparison');
...
if (legacyWindow.AlbEdu && legacyWindow.AlbEdu.icon) {
  ...
  // Current: 10000 renders (already warmed above)
  ...
  console.log(`  legacy avg: ${fmt(legacyAvg)}ms/render`);
  console.log(`  current     avg: ${fmt(v7Avg)}ms/render`);
  ...
  console.log('  (legacy icons not available for comparison)');
}
