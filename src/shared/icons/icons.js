// =============================================================================
// icons.js — AlbEdu Shared Layer · Lucide Icon System (v5.0)
// =============================================================================
// Single responsibility: render icons via Lucide (https://lucide.dev) — a
// modern, MIT-licensed SVG icon set used by Vercel, shadcn/ui, Cal.com,
// Linear, and other top-tier SaaS products.
//
// === WHY LUCIDE (v5.0) ===
//
// Previous versions:
//   v2.x — custom SVG registry (98 manually maintained icons)
//   v3.0 — Material Symbols font from Google CDN (3.8MB, FOUT risk)
//   v4.0 — Material Symbols self-hosted subset (6.3KB, but font-based)
//
// v5.0 switches to Lucide because:
//   1. ZERO font loading — SVG paths are inlined in JS bundle, rendered
//      instantly on first paint. No font fetch, no FOUT, no FOUC.
//   2. Smaller bundle — 88 icons = ~12KB total (vs 22KB for v4 font+JS)
//   3. CSS-controllable — full control over stroke-width, color, hover
//      effects, animations. Material Symbols only exposes variable-font axes.
//   4. Modern aesthetic — consistent 2px rounded stroke, "premium SaaS"
//      look that matches Vercel/Linear/shadcn/ui design language.
//   5. Active maintenance — Lucide team adds new icons weekly, ISC license.
//
// === ARCHITECTURE ===
//
// Registry: { 'icon_name': '<svg_inner_content>' }
//   - Keys are AlbEdu icon names (underscore form, e.g. 'account_circle')
//   - Values are the INNER content of Lucide SVGs (paths, circles, lines)
//   - The wrapper <svg> with viewBox/stroke/etc is added by icon() at render
//
// Naming normalization (backward compatible with v3/v4):
//   - underscore  : 'account_circle'  (canonical)
//   - hyphen      : 'account-circle'  (auto-converted)
//   - camelCase   : 'rocketLaunch'    (auto-converted)
//
// Public API (unchanged from v3/v4 — fully backward compatible):
...
// === USAGE ===
//
//   HTML: <span data-albedu-icon="account_circle"></span>
//   JS:   AlbEdu.icon('login', { size: 20 })
//   JS:   AlbEdu.icon('home', { class: 'nav-icon', 'aria-label': 'Home' })
//
// === SVG PROPERTIES ===
//
// All icons share:
...
//   - stroke-width: 2 (override via CSS .albedu-icon { stroke-width: 1.5 })
//   - stroke-linecap: round
//   - stroke-linejoin: round
//
// Sizing:
//   - Default: 1em × 1em (scales with font-size)
//   - Explicit: AlbEdu.icon('login', { size: 20 }) → 20px × 20px
//   - CSS class: .albedu-icon--16 { width: 16px; height: 16px }
//
// Coloring:
//   - Inherits currentColor — set CSS `color` on parent or icon itself
//   - Example: .nav-icon:hover { color: var(--color-primary) }
//
// === ICONS NOT IN LUCIDE ===
//
// If an icon used in the codebase is not in Lucide (rare), it falls back
// to empty string + console warning. Use AlbEdu.registerIcon() to add a
// custom SVG path at runtime if needed.
//
// Failed icons from build (none expected — all 88 should resolve):
//   (none)
// =============================================================================
...
(function () {
  ...
  // ── Icon registry ──────────────────────────────────────────────────────
  // All icons use 24×24 viewBox, stroke=currentColor, stroke-width=2,
  // stroke-linecap=round, stroke-linejoin=round (set on the SVG wrapper by
  // icon() at render time).
  //
  // Values are the INNER SVG content (paths, circles, lines) — extracted
  // from Lucide's official SVG files. The wrapper <svg> with all attributes
  // is added by icon() at render time so we can apply opts (size, class,
  ...
  // ── Name normalization ──────────────────────────────────────────────────
  // Accepts all 3 input conventions, normalizes to underscore form (which
  // is what the registry keys use).
  ...
  //   AlbEdu.icon('home', { strokeWidth: 1.5 })  // override default 2px
  function icon(name, opts) {
    ...
    if (!path) {
      if (window.console && console.warn) {
        console.warn('[albedu:icons] unknown icon:', name,
                     ...
                     '— register via AlbEdu.registerIcon() if custom.');
      }
      return '';
    }
    opts = opts || {};
    ...
    var strokeWidth = opts.strokeWidth != null ? opts.strokeWidth : null;

    var attrs = 'class="albedu-icon ' + classes + '"' +
                ' viewBox="0 0 24 24" fill="none" stroke="currentColor"' +
                ' stroke-width="' + (strokeWidth != null ? strokeWidth : 2) + '"' +
  ...
  function setIcon(el, name, opts) {
    if (!el) return;
    var html = icon(name, opts);
    if (!html) {
      el.innerHTML = '';
      return;
    }
    el.innerHTML = html;
    // Preserve any existing classes on the element by appending them to the
    // SVG wrapper. (bindIcons uses this — element class becomes the icon class.)
    // Note: setIcon replaces innerHTML, so we re-add element's own classes
    // to the SVG via classList if they aren't already there.
    if (el.className && typeof el.className === 'string') {
      var svgEl = el.querySelector('svg.albedu-icon');
      if (svgEl) {
        var existingClasses = el.className.split(/\s+/).filter(Boolean);
        existingClasses.forEach(function (c) {
          if (c && c !== 'material-symbols-outlined' && c !== 'albedu-icon') {
            svgEl.classList.add(c);
          }
        });
      }
    }
  ...
  // ── Public API: registerIcon() ──────────────────────────────────────────
  // Register a custom SVG icon at runtime. svgPath is the INNER content of
  // an SVG (paths, circles, lines) — same format as registry values.
  ...
  // ── Public API: bindIcons() ─────────────────────────────────────────────
  // Scans a root element for [data-albedu-icon="name"] attributes and
  // materializes them as inline SVG icons.
