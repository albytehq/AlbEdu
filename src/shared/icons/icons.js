...
//   AlbEdu.getMetrics()                   → performance metrics (debug)
...
(function () {
  ...
  function _emit(event, detail) {
    ...
    for (var i = 0; i < listeners.length; i++) {
      try {
        ...
        _logError('event-listener:' + event, e);
      }
    }
  }
  ...
  function addEventListener(event, cb) {
    ...
    return function unsubscribe() {
      var idx = _eventListeners[event].indexOf(cb);
      if (idx !== -1) _eventListeners[event].splice(idx, 1);
    };
  }
  ...
  function _logError(context, err) {
    ...
    _metrics.errors.push(entry);
    if (_metrics.errors.length > _MAX_ERRORS) {
      _metrics.errors.shift();
    }
  ...
  function icon(name, opts) {
    try {
      ...
      if (!resolved.path) {
        _metrics.missingIcons[resolved.name] = (_metrics.missingIcons[resolved.name] || 0) + 1;
        _emit('icon-missing', { requested: name, normalized: resolved.name });
        ...
        return _buildSvg(FALLBACK_SVG, opts, 'albedu-icon albedu-icon--missing');
      }

      _metrics.iconsRendered++;
      return _buildSvg(resolved.path, opts, 'albedu-icon');
    } catch (err) {
      _logError('icon:' + name, err);
      _emit('icon-error', { name: name, error: err });
      if (opts && opts.fallback === false) return '';
      return _buildSvg(FALLBACK_SVG, opts || {}, 'albedu-icon albedu-icon--error');
    }
  }

  function _buildSvg(path, opts, classes) {
    ...
    var allClasses = classes + ' ' + extraClasses;
  ...
  function setIcon(el, name, opts) {
    ...
    try {
      ...
      _logError('setIcon:' + name, err);
      _emit('icon-error', { name: name, error: err, element: el });
    }
  }
  ...
  function registerIcon(name, svgPath) {
    ...
    try {
      I[_normalizeName(name)] = svgPath;
      return true;
    } catch (err) {
      _logError('registerIcon:' + name, err);
      return false;
    }
  }
  ...
  function getMetrics() {
    return {
      ...
      iconsBound: _metrics.iconsBound,
      missingIcons: Object.assign({}, _metrics.missingIcons),
      missingIconCount: Object.keys(_metrics.missingIcons).length,
      errorCount: _metrics.errors.length,
      errors: _metrics.errors.slice(),
      bindTimeMs: _metrics.bindTimeMs,
      initTimeMs: _metrics.initTimeMs,
      lastBindTimestamp: _metrics.lastBindTimestamp,
      totalIconsInRegistry: Object.keys(I).length,
    };
  }

  function resetMetrics() {
    _metrics.iconsRendered = 0;
    _metrics.iconsBound = 0;
    _metrics.missingIcons = {};
    _metrics.errors = [];
    _metrics.bindTimeMs = 0;
    _metrics.initTimeMs = 0;
  ...
  function _bindNode(node) {
    ...
    var normalizedName = _normalizeName(rawName);
    if (_SPRITE[normalizedName] && document.getElementById('i-' + normalizedName)) {
      var useHtml = '<svg class="albedu-icon ' + existingClass + '"' +
                    ' viewBox="0 0 24 24" fill="none" stroke="currentColor"' +
                    ' stroke-width="2" stroke-linecap="round" stroke-linejoin="round"' +
                    ' aria-hidden="true"><use href="#i-' + normalizedName + '"/></svg>';
      node.innerHTML = useHtml;
      _metrics.iconsBound++;
      return;
    }

    setIcon(node, rawName, { class: existingClass });
    _metrics.iconsBound++;
  }
  ...
  function bindIcons(root) {
    ...
    try {
      ...
      _metrics.bindTimeMs = performance.now() - startTime;
      _metrics.lastBindTimestamp = Date.now();
      _emit('icons-bound', {
        immediate: result.immediate,
        deferred: result.deferred,
        durationMs: _metrics.bindTimeMs,
      });
    } catch (err) {
      _logError('bindIcons', err);
      _emit('icon-error', { context: 'bindIcons', error: err });
    }
  ...
  function _autoInit() {
    ...
    try {
      ...
      _metrics.initTimeMs = performance.now() - startTime;
    } catch (err) {
      _logError('autoInit', err);
    }
  }
