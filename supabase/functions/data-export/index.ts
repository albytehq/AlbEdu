// data-export/index.ts — Self-service data export for UU PDP compliance.
// POST /functions/v1/data-export
// Headers: Authorization: Bearer <token>
// Returns JSON of all the caller's personal data.
...
export default handler(async (req: Request, env: Env, _ctx: any) => {
  ...
  const [profile, submissions, violations, auditLogs, consents, dsrs] = await Promise.all([
    ...
    // Violation events (90-day retention window per the purge pg_cron job).
