...
--
-- Q9: audit trail tier B (violation events tracked).
-- Q10: retention 90 days (auto-purge via pg_cron — see 013).
-- Q12: heartbeat 15s + DevTools detection. Violations pushed realtime to admin.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS public.violation_events (
  ...
  -- Forensics (Q17 compliance — IP hashed after 90 days)
  ip_address      text,
  user_agent      text,
  device_id       text,

  -- Auto-expiry (Q10: 90 days)
...
COMMENT ON TABLE public.violation_events IS
  'Normalized violation log (1 row per event). Replaces violations table with embedded JSONB array. Q9: audit trail. Q10: 90-day retention.';
