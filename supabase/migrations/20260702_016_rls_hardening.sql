...
-- AlbEdu Phase 4 — Security hardening of RLS policies
-- =============================================================================
...
--    without scope. Should be limited to grading fields only (Phase 9
--    future manual esai grading). For now, restrict to graded_by/graded_at/
--    grading_notes columns via a COLUMN-level policy.
--
-- FIXES:
-- =============================================================================

-- ── Fix 1: Replace peserta UPDATE policy on assessment_sessions ────────
...
-- ── Fix 2: Restrict violation_events INSERT to peserta's own user_id ──
...
-- ── Fix 4: Restrict submissions admin UPDATE to grading fields only ──
-- Phase 9 will add manual esai grading. Until then, no UPDATE at all.
...
-- Re-add when Phase 9 lands. For now, submissions are immutable post-submit.
-- Score is computed server-side in submit-assessment Edge Function.

-- ── Fix 5: Add audit_logs INSERT policy for Edge Functions ──────────
...
-- ── Fix 6: Add comment documenting the policy hardening ─────────────
COMMENT ON POLICY "sessions_peserta_update_own_safe_fields" ON public.assessment_sessions IS
  'Phase 4 hardening: peserta can only update progress fields (heartbeat, draft_answers, current_section/question, progress_pct). All sensitive state (status, blocked_at, submitted_at, started_at, violation_count, attempt_number) is immutable from the client — must go through Edge Functions using service role key.';
