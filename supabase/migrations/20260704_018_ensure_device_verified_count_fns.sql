...
-- Defines count_verified_users_by_device and count_verified_admins_by_device.
-- register-admin/index.ts and user-auth-complete/index.ts both call these RPCs
-- to enforce the 2-account device limit, but neither function was defined
-- anywhere in migrations/. supabase/README.md listed both as "Legacy, kept
-- for ..." — likely they were created directly via the SQL editor and never
-- captured in the migration set.
--
-- This migration is idempotent (CREATE OR REPLACE). It is safe whether or not
-- the functions already exist — it won't break anything if they're already
-- present with the same behavior, and it closes the gap if they were never
-- created (which would cause EVERY new user/admin registration to fail with
-- a 500, since both edge functions treat an RPC error as fail-closed).
--
-- "Verified" here = the account has confirmed its email
-- (auth.users.email_confirmed_at IS NOT NULL). Counts DISTINCT users linked
-- to the given device_id via user_devices, filtered by public.users.peran.
...
-- Revoke from anon/authenticated defensively since SECURITY DEFINER functions
